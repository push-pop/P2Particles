...
//								SHADOW_COORDS(5)
					...
					v2f vert(VS_INPUT v)
					{
						...
						float3 angle2 = p.pPos - p.pos;
					    
						float3 axis = normalize(cross(norm, angle));
						float3 axis2 = normalize(cross(norm, angle2));
						//float3 vert = _Scale*meshData[meshData[mIndex].index].vert;// *float3(o.uv.x + p.vel.x * 100, o.uv.y + p.vel.y * 100, 0);
						float3 VA = lerp(p.pos + axis * (.5 - o.uv.y) *_Scale, p.nPos + axis * (.5 - o.uv.y) *_Scale, o.uv.x);
						float3 VB = lerp(p.pos + axis2 * (.5 - o.uv.y) *_Scale, p.nPos + axis2 * (.5 - o.uv.y) *_Scale, o.uv.x);
						float3 vert =  lerp(VA,VB,1-o.uv.x);// meshData[meshData[mIndex].index].vert;
						//vert = mul(rot, vert);
					...
					//float d = distance(float3(0, 0, 0), i.color.xyz);
					//u *= (1-d*.2);
					/*
						float n = abs(snoise(i.color.xyz));
						float n2 = abs(snoise(i.color.xyz+n*u*.3));
						u *= n2;*/
						return float4(u,u,u,u) * _Color * _Color.a * 5;// tex2D(_MainTex, i.uv)*i.color;
					}
